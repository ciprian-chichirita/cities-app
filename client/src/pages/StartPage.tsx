...
import { City, Cities } from '../models';
...
export const StartPage = () => {
  ...
  const [filterByContinentOptions, setFilterByContinentOptions] =
    useState<Array<string> | null>(null);
  const [show, setShowModal] = useState(false);
  const [position, setMapPosition] = useState<LatLngExpression>([
    44.4268, 26.1025,
  ]);
  ...
  const filterEventHandler = useCallback((event) => {
    if (event !== 'all') {
      setCities(
        [...(citiesRef.current ?? [])].filter(
          (city) => city.continent === event
        )
      );
    } else {
  ...
  useEffect(() => {
    client('cities', {}).then(
      (data: Cities) => {
        ...
        setCities(cities);
        const filterByContinentOptions = cities.reduce(
          (
            acc: Array<string>,
            crtElem: City,
            pos: number,
            ...
            if (
              self.findIndex((elem) => elem.continent === continent) === pos
            ) {
              acc.push(continent);
              return acc;
            }
  ...
  const renderCardText = (city: City | null, ommit: Array<string> = []) => {
    ...
    if (ommit.length) {
      cardTextPropsArr = cardTextPropsArr.filter(
        (elem) => ommit.indexOf(elem) === -1
      );
    }
    if (!city) {
      return null;
      ...
      if (!Array.isArray(city[key as keyof typeof city])) {
        return (
          <span key={key} className='d-block'>
            <strong>{key}:</strong> {city[key as keyof typeof city]}
          </span>
        );
      }
      return (
        <span key={key} className='d-block'>
          <i>
            <strong>{key}:</strong>{' '}
            {(city[key as keyof typeof city] as Array<string>).join(', ')}
          </i>
        </span>
      );
    });
  ...
  const renderCards = (citiesArr: Array<City>) => {
    return citiesArr.map((city: City, index: number) => {
      return (
        <Card key={`${city.founded}_${index}`} style={{ width: '18rem' }}>
          <Card.Body className='d-flex flex-column'>
            <Card.Title>{city.name}</Card.Title>
            <Card.Subtitle className='mb-2 text-muted'>
              {city.name_native}
            </Card.Subtitle>
            <Card.Text>{renderCardText(city)}</Card.Text>
            <Button
              data-testid={`button-show-map-${city.name
                .replace(/\s/gi, '_')
                .toLowerCase()}`}
              variant='primary'
              className='mt-auto'
              onClick={() => openMap([+city.latitude, +city.longitude], city)}
            >
              Show on map
            </Button>
          </Card.Body>
      ...
      <main>
        ...
        <Modal show={show} fullscreen={true} onHide={() => setShowModal(false)}>
          <Modal.Header closeButton>
            <Modal.Title
              data-testid={`modal-title-${city?.name
                .replace(/\s/gi, '_')
                .toLowerCase()}`}
            >
              {city?.name}
